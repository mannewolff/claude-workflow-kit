...
**Die Zusammenfassung begleitet den Lauf.** Sie entsteht **vor** dem ersten Kommando, wird **vor** jedem weiteren überschrieben und ein letztes Mal am Ende geschrieben. Nur diese letzte Fassung trägt `abgeschlossen: true`; in jeder anderen steht das gerade laufende Kommando noch auf `nicht gestartet`. Daraus folgt, was ein Abbruch hinterlässt — stirbt der Lauf an der Uhr, durch ein Signal oder mit seiner Session, liegt eine Fassung da, die **nie ganz grün** ist. Vorher fehlte die Datei in diesem Fall ganz, und eine fehlende Datei heißt „ungeprüft", also ungemessen statt beanstandet. Gate und Nacht-Runner werten `ergebnis` aus und nicht `abgeschlossen`: Der Zeitpunkt des Schreibens ist die eigentliche Regel, das Feld sagt nur, ob der Lauf sein Ende erreicht hat.
...
**Neun Teile.** `nachtrunner`, `board`, `pruefungen`, `einstellungen`, `worktree`, `installer`, `skills-doku`, `konfiguration` und `preise`. Sie überschneiden sich absichtlich, aber sparsamer als früher: Seit Issue #933 beansprucht **kein** Teil mehr das ganze Werkzeugverzeichnis — das Muster `kit/**` stand einmal sowohl im `installer` als auch in `skills-doku` und zog damit bei jeder Änderung an einer beliebigen Kit-Datei dieselben Doku-, CLI-, Sync- und Installer-Tests herbei. Stattdessen trägt jede Quelldatei unter `kit/` den Teil ihrer gemessenen Verflechtung, und seit Issue #936 ist `kit/preise.mjs` ein Teil für sich statt in drei Teilen zugleich. Ein Teil ist kein Eigentum an einer Datei, sondern die Antwort auf die Frage „welche Prüfung muss laufen, wenn sich das hier ändert?" — und auf die darf mehr als ein Teil mit Ja antworten.

**Sechs Testaufrufe, nicht einer je Teil.** Die Testsuite zerfällt nicht in unabhängige Stücke, weil die Kit-Werkzeuge einander laden: `kit/night.mjs` zieht `board.mjs`, `checks.mjs`, `aufwand.mjs`, `wirksamkeit.mjs` und `befunde.mjs` nach, `.githooks/gate.mjs` lädt `checks.mjs`, und knapp fünfzig Nacht-Tests nennen `board.mjs`. Dazu kommt, dass `checks.mjs run` die Einträge **nacheinander** fährt: Jeder zusätzliche Eintrag verlängert den vollen Umfang um den Startaufwand eines weiteren Laufs, der seine Dateien nicht mehr mit den anderen parallelisiert. Die Aufteilung grenzt darum gröber ein, als die Teile es zuließen — sie bleibt eine Abwägung zwischen dem Gewinn im eingegrenzten Fall und dem Aufschlag im vollen Umfang.

**Was ohne Teil bleibt, ist ebenfalls eine Aussage.** Seit Issue #935 ist jede versionierte Quelldatei zugeordnet — auch `eslint.config.mjs`, `package.json`, `.claude/workflow.config.json` (eigener Teil `konfiguration`) und jeder Helfer unter `test/helpers/**`, der in den Teilen steht, deren Tests ihn laden. Was bleibt, sind die Dateien, zu denen es wirklich nichts zu prüfen gibt: Sie stehen mit eigenem Grund in [`ohnePruefung`](#ohnepruefung) — `CHANGELOG.md`, `LICENSE`, die Lizenztexte der Schriften, die Notizdokumente der Wurzel. Wer eine Datei weder in einen Teil noch in die Freistellung setzt, bekommt bei jeder Änderung an ihr den vollen Umfang; das ist die sichere Richtung und keine Strafe.

**Ein Schutztest gegen die Lücke.** Mit den aufgeteilten Aufrufen entsteht eine neue Fehlerart, und sie ist still: Eine Testdatei mit einem Präfix, das keiner der Aufrufe nennt, läuft **nirgends** — nicht vor dem Commit, nicht im Commit-Gate, nicht in der CI. Grün hieße dann „nicht geprüft". `test/config-teile.test.mjs` prüft deshalb die echte Config gegen `git ls-files`: jede versionierte Testdatei von mindestens einem Aufruf erfasst, jede versionierte Quelldatei in mindestens einem Teil **oder** in `ohnePruefung`, und keine Datei zugleich in beidem — eine Freistellung, die ein Teil überstimmt, verspricht im Bericht etwas, das nicht gilt. Die Glob-Auflösung holt der Test aus `kit/checks.mjs` statt sie nachzubauen — ab der ersten Abweichung bescheinigte eine zweite Fassung eine Abdeckung, die das ausführende Kommando nicht sieht.
...
- `buildChecks[].nichtBeimAbschluss` — Warum diese Prüfung den Abschluss eines einzelnen Arbeitspakets nicht tragen muss: "zusammenspiel" = sie prüft das Zusammenspiel mehrerer Teile, "volleTestmenge" = sie gewinnt ihren Befund aus der vollständigen Testmenge. Die vierte Achse neben "areas"/"always" (ob eine Prüfung betroffen ist), "stufe" (wann sie an der Reihe ist) und "guete" (was sie misst) — dieses Feld sagt nichts davon, sondern warum sie beim Abschluss entfallen darf. Sie wirkt nur, wenn der Aufruf "--abschluss" trägt: den setzen allein die implement-Skills beim Abschluss genau einer Karte. Jeder andere Lauf fährt die Prüfung mit, und spätestens vor dem Veröffentlichen läuft sie in jedem Fall. Nur an einem Eintrag der Stufe "paket" sinnvoll — an "push" oder "merge" läuft die Prüfung beim Abschluss ohnehin nicht, und der Prüflauf bricht dort ab. Trägt jeder Eintrag der Paketstufe dieses Feld oder einen guete-Block, hat der Abschluss kein Gate mehr; auch das bricht ab. Fehlendes Feld = die Prüfung läuft auch beim Abschluss = unverändertes Verhalten. Gilt teamweit; ein abweichender Wert in workflow.config.local.json wird ignoriert. (gültig: `zusammenspiel`, `volleTestmenge`)
...
- `night.kette` — Budgets und Kennzeichen der Nacht-Kette (night.mjs --kette). Eine fachliche Anforderung oder ein Plandokument mit dem Label geht abends hinein; jede Zahl ist ein Abbruchgrund mit Grund im Bericht, kein Fehler des Prozesses.
- `night.kette.label` — Das Kennzeichen an der fachlichen Anforderung oder am Plandokument, das die Kette startet. Jedes Setzen autorisiert genau eine Kette; der Start verbraucht es.
- `night.kette.varianteBLabel` — Das Kennzeichen, das die gekennzeichnete Karte für die Umsetzungsstufe der Kette (Variante B) kennzeichnet.
...
**Schritt 4: das GO.** Du entscheidest, welche Issues in diesen Batch kommen. Darin liegt die Planung: wie viel Arbeit auf einmal, welche Priorität, welche Abhängigkeiten. Das GO hat zwei Körnungen: Unter **Variante A** ziehst du jedes Arbeitspaket einzeln nach Ready. Unter **Variante B** kennzeichnest du stattdessen vorab die Karte, die die Kette startet — die fachliche Anforderung oder das Plandokument —, mit dem Label aus `night.kette.varianteBLabel` (siehe [Zweiter Modus](#zweiter-modus-die-nacht-kette)) — das GO gilt dann im Voraus für alle Arbeitspakete, die die Nacht-Kette daraus schneidet; die Kette zieht sie selbst nach Ready und setzt sie in derselben Nacht um.
...
**Das Live-Verlaufsprotokoll ist der Normalfall und braucht kein Flag.** Der Runner liest den `stream-json`-Output der Session live mit und schreibt kompakte Ereigniszeilen ins Nacht-Log und auf die Konsole — Tool-Aufrufe und Text-Snippets, jeweils mit der Issue-Nummer:
...
Die finale Abschlussnachricht landet wie gehabt zusätzlich im Log; das Streaming ergänzt sie, ersetzt sie nicht. Abgeschaltet wird es ausdrücklich mit `--verbose no` — dann protokolliert der Runner pro Runde nur Start und Ende, und bei einer langen Session sieht man nicht, woran sie gerade arbeitet (`claude -p` gibt erst am Schluss seine Abschlussnachricht aus). Das Flag zu vergessen kostete das Verlaufsprotokoll früher genau in der Nacht, in der man es braucht; deshalb ist die Vorbelegung seit Issue #867 umgedreht. Ein bloßes `--verbose` bleibt zulässig und wirkungslos, damit bestehende Aufrufe und Skripte unverändert weiterlaufen.

**Der Ergebnisstand — die Nacht als JSON.** Jeder Lauf legt neben dem Textprotokoll (`.claude/night-run-<datum>.log`) einen maschinenlesbaren Ergebnisstand unter `.claude/night-run-<datum>-<uhrzeit>.json` ab: je Arbeitspaket eine Einheit mit Ausgang, Dauer, Commit und den Kennzahlen der Session (Kosten, API-Dauer, Züge), dazu der Abschluss des ganzen Laufs (`regulaer` oder `harterStopp`, im Stoppfall mit Fehlerklasse und Grund). Als Ausgang einer Einheit kommen vor: `erfolg`, `zurueckgestellt`, `angehalten` (siehe unten), `uebersprungen`, `liegengeblieben` und `harterStopp`. Die Datei wird nach jeder Runde vollständig neu geschrieben, ein abgebrochener Lauf hinterlässt also den Stand bis zum Abbruch. **Verbrauch:** Die Kennzahlen jeder Session tragen neben Kosten, API-Dauer und Zügen die vier Token-Mengen aus der CLI — `eingabeTokens`, `ausgabeTokens`, `cacheErzeugtTokens`, `cacheGelesenTokens`. Jede Einheit summiert sie als `verbrauch` über alle ihre Sessions (auch Salvage und Ketten-Stufen) und nennt ihre Lauf-Art als `art`; der Lauf-Kopf führt `verbrauch` über alle Sessions des Laufs, einschließlich des Vorflugs, und `verbrauchOhneEinheit` als den Teil, der zu keiner Karte gehört. Ein Feld, zu dem nie eine Menge kam, bleibt `null` — eine 0 behauptete, es sei nichts verbraucht worden. **Prüfläufe:** Neben `zeiten` trägt jede Einheit `prueflaeufe` mit dem Block `arbeit` — was die Session während ihrer Arbeit an Prüfläufen gestartet hat: `anzahl` alle (ein Bash-Aufruf, dessen erstes Programm das einer konfigurierten Prüfgruppe ist), `volle` davon die, die wörtlich einem Prüfgruppen-Kommando entsprachen, `volleNoetig` die sanktionierten Gruppenläufe über `checks.mjs run --bereich <name>`, und `dauerMs` die Summe ihrer Spannen. Mehrere Sessions einer Karte addieren sich wie die Zeiten. Der Abschlussversuch — `checks.mjs run` ohne `--bereich` — steht bewusst **nicht** in `anzahl`, sonst zählte er doppelt: Er hat seinen eigenen Block `abschluss` neben `arbeit`, mit `anzahl` und `dauerMs`. Ein übernommener Lauf (`Ergebnis uebernommen`, weil der Stand unverändert ist) zählt dort als Versuch **ohne** Dauer — `checks.mjs` reicht die Werte des früheren Laufs weiter, eine Spanne dieses Aufrufs wäre geerbt und nicht gemessen. Eine Session ohne angeforderten Strom (Kommando-Stufe mit fremdem Programm) trägt nichts bei; hat keine Session der Karte gemessen, fehlt das Feld ganz — „nicht gemessen" ist etwas anderes als „keiner". Gespeichert wird allein die Messung: Was daraus folgt — Läufe je Abschluss, Anteil an der Laufzeit, Abstand zur Zielmarke `night.zielUmsetzungMin` — rechnet der Bericht. `complete` steht in der Datei auf `false`, solange der Lauf läuft, und wird nur am regulären Ende `true` — ein harter Stopp lässt es dort auf `false` stehen (die Meldung ans Board sagt darüber etwas anderes, siehe den Absatz „Einlieferung ans Board" weiter unten). Die Uhrzeit gehört in den Namen, weil das Textprotokoll eine Tagesdatei zum Anhängen ist, JSON aber nicht angehängt werden kann — der zweite Lauf eines Tages überschriebe sonst den ersten. **Der einzige Ausschluss ist `--dry-run`**: Ein Dry-Run arbeitet nichts ab und hat nichts zu berichten.

**Die Prüfläufe und die Zielmarke stehen in beiden Berichten.** Eine Messung nützt nur, wenn sie jemand liest, und die Nacht berichtet an zwei Orten: Die Umsetzungsnacht schreibt ihren Prüfbericht auf Konsole und ins Tagesprotokoll, die Kette hinterlässt ihren Nachtbericht als Kommentar an der gekennzeichneten Karte. Der Block `Prueflaeufe und Zielmarke:` steht an **beiden** Stellen — im Protokoll unter den Prüfungen der Sessions, im Nachtbericht unter `### Umsetzung` — und trägt je Paket eine Zeile mit der Rundendauer, der Zahl der Prüfläufe, davon `volle` und `Gruppenlaeufe`, und der Zahl der Abschlussversuche. Hat keine Session der Karte gemessen, sagt die Zeile `Prueflaeufe nicht gemessen` und nennt keine Null. Darunter die Summe: `- N von M Paketen unter <marke> Minuten.` — die Marke ist `night.zielUmsetzungMin`, ohne Feld gilt die Vorgabe 10. Gezählt werden nur die Einheiten, die eine Implementierungs-Runde durchlaufen haben; die Fachplan-Einheit der Kette und Pakete ohne Session bleiben draußen. Gerechnet wird mit der Rundendauer einschließlich aller Prüfungen und Korrekturen, und gerechnet wird im Bericht — eine zweite Ablage in der Datei wäre dieselbe Wahrheit an einem zweiten Ort. **Lücken in der Zuordnung** stehen daneben: Trug die Prüf-Zusammenfassung eines Pakets Dateien ohne Bereichsmuster (`ohneZuordnung`), nennt der Bericht **jede** davon mit Namen. Das färbt nichts rot — der Abschluss des Pakets bleibt unberührt, es ist ein Befund für den Morgen, dass ein Bereich ein Muster braucht.

**Weder die Datei noch die Kennzahlen hängen an einem Flag.** Der Runner fordert die ausführliche Session-Ausgabe (`stream-json`) in jedem unbeaufsichtigten Lauf an und kommt damit immer an Kosten, API-Dauer und Züge heran — auch bei `--verbose no`, das allein steuert, ob die Ereignisse zusätzlich auf Konsole und Tagesprotokoll erscheinen. Ein Feld `kennzahlenHinweis` gibt es deshalb seit Issue #668 nicht mehr. Früher hing die ganze Datei am Flag — das kostete die Auswertung in genau der Nacht, in der jemand es vergessen hatte, und das ist die Nacht, in der man sie braucht: **Der Grund eines Abbruchs wiegt mehr als die Kennzahlen eines glatten Laufs.** Zur Konsequenz gehört, dass auch ein Lauf, der schon am **Vorflug** scheitert — Crash-Rest in *In progress*, unsauberer Working Tree, leere `buildChecks`, Reviewer-Vorflug —, einen Ergebnisstand mit `abschluss: "harterStopp"` und Fehlerklasse hinterlässt, obwohl er kein einziges Paket abgearbeitet hat. Genau dort sucht man morgens den Grund.
...
**Ein Lauf ohne Arbeit nennt seinen Grund.** Endet eine Nacht, ohne dass ein einziges Arbeitspaket an der Reihe war, trägt der Lauf-Kopf dafür das Feld `noWorkReason` — wörtlich mit dem Satz, der ohnehin ins Textprotokoll geht. Es gibt sechs benannte Fälle. In der **Umsetzungsnacht**: Ready ist leer (`Ready ist leer — nichts zu tun.`), keines der Ready-Issues trägt das Routing-Label (`Keine der <N> Karten in Ready traegt das Label '<name>' — nichts zu tun.`), alle Karten wurden am Gate zurückgestellt (`Alle <N> Karten in Ready wurden am Gate zurueckgestellt — …`) oder die Umsetzung ist durch einen anderen Lauf belegt (`Die Umsetzung ist belegt: <grund> — der Lauf endet ohne Paket.`). Bei `--kette`: keine Karte trägt das Kettenlabel (`Keine Kette zu fahren: keine Karte traegt das Label '<name>'.`) oder alle wurden übersprungen, weil eine Voraussetzung fehlt (`Keine Kette zu fahren: alle <N> gekennzeichneten Karten mit dem Label '<name>' wurden uebersprungen, …`). Jeder Satz führt Label und Zahl beziehungsweise den haltenden Prozess mit, damit der Morgen den Fall ohne Rückfrage erkennt und nicht erst das Textprotokoll daneben lesen muss.

**Der Rückstellungs-Satz hat Vorrang vor `Ready ist leer`.** Hat der Lauf Ready selbst geräumt, indem er jede Karte am Gate zurückgestellt hat, sagt er genau das — und behauptet nicht, Ready sei von vornherein leer gewesen. Und ein Lauf, der ohne Session endete, ohne dass einer der sechs Fälle zutraf, schweigt nicht, sondern trägt den Rückfall-Satz `Kein Grund ermittelbar — … bitte melden.`: Ein leeres Feld ließe offen, ob nichts zu sagen war oder eine Lage unbenannt blieb.

**Ein nicht schreibbarer Umsetzungs-Lock ist kein ruhiger Lauf.** Lässt sich die Lock-Datei nicht schreiben, ist das eine Störung der Umgebung: Der Lauf endet als harter Stopp der Fehlerklasse `umgebung`, der Grund steht am `fehlerText` des Laufs, und `noWorkReason` bleibt aus — ohne `noWorkReason` ist morgens auf einen Blick zu sehen, dass hier etwas gerissen ist. Ein bloß **belegter** Lock dagegen bleibt ein ruhiger Lauf mit Grund (`Die Umsetzung ist belegt: …`). Ohne das Feld sah eine leere Nacht in der Auswertung aus wie eine abgebrochene: keine Einheit, kein harter Stopp, kein Hinweis darauf, dass schlicht nichts zu tun war. Ein Lauf mit Arbeit trägt es nicht.

**Einlieferung ans Board.** Mit `issueTracker: toolbox` liefert der Runner den Ergebnisstand fortschreibend an kanban-kit ein — nach jeder Einheit und am Ende, über `board.mjs nightrun melden` an `POST /api/kanban/night-runs`, mit dem projektgebundenen Token. **Die erste Meldung geht raus, bevor das erste Arbeitspaket dran ist:** unmittelbar nachdem der Ergebnisstand angelegt ist, mit leerer Paketliste und `complete: false`. Ein Lauf, der schon im Vorflug oder in der ersten Session stehenbleibt, ist damit am Board zu sehen — vorher war er von einer Nacht, die nie gestartet wurde, nicht zu unterscheiden. Die Startmeldung erzeugt keine Protokollzeile, und im Dry-Run entfällt sie wie jede andere Meldung. kanban-kit ersetzt denselben Lauf bei jeder Meldung. **Ein harter Stopp geht als abgeschlossener Lauf ein:** Die Meldung trägt `complete: true` samt `abortReason` — dem Grund des Stopps, gekürzt auf die 4.000 Zeichen des Vertrags —, und das Board wertet den Lauf damit als Störung und zeigt den Grund an. Die **Datei** behält davon unberührt `complete: false`, denn der Lauf kam nie durch. Vorher meldete den Stopp niemand: Die Nacht blieb am Board unabgeschlossen unter den aktiven Läufen stehen, obwohl sie lange tot war. Der Grund entsteht als Kaskade — `fehlerText` des Laufs, sonst der `grund` der über `fehlerEinheit` bezeichneten Einheit, sonst „Harter Stopp" mit der Fehlerklasse. Eine gerissene einzelne Kette ist **kein** Abbruch des Laufs: Sie endet regulär und meldet keinen `abortReason`, ihr Ausgang steht am roten Paket. Dafür muss das Board mindestens auf kanban-kit **v2.7.0** stehen — ein älteres verwirft das unbekannte Feld still und zeigte den Stopp als grün abgeschlossene Nacht. Scheitert die Einlieferung, endet der Lauf **nicht** als Fehlschlag: Das Protokoll nennt den Grund, und die Datei bleibt der Rückfall. Mit jedem anderen Tracker entfällt die Einlieferung mit einer Protokollzeile.
...
**Was geprüft wurde — und was nicht.** Die Sessions prüfen bereichsbezogen (siehe [Bereichsbezogene Prüfungen](#bereichsbezogene-prüfungen-checkareas)), und die Auslassungen sind an **zwei** Stellen sichtbar: im **Abschlussbericht am Arbeitspaket**, wo die Session gelaufene und ausgelassene Prüfungen jeweils mit Grund aufführt, und im **Lauf-Bericht des Durchgangs** — dort je Session eine Zeile und darunter eine Summenzeile über den ganzen Lauf. Eine Session ohne Prüfung erscheint ausdrücklich als „ungeprüft", ein Paket ohne Änderungen als „leeres Paket"; nichts davon versteckt sich hinter einer leeren Liste. Eine Session, die **während** des Prüflaufs stirbt, erscheint als **rot** und nicht mehr als „ungeprüft": Seit die [Zusammenfassung den Lauf begleitet](#das-commit-gate), bleibt in diesem Fall eine unabgeschlossene Fassung mit `nicht gestartet` liegen. Die Verschiebung ist gewollt — ein gestorbener Prüfer ist eine Beanstandung, keine ausgefallene Messung. Der Runner liest das nicht aus dem Abschlussbericht, sondern aus der Zusammenfassung, die `checks.mjs` in `.claude/checks-summary.json` hinterlässt: Was die Maschine auswertet, geht hier nirgends durch von einem Modell formulierten Text.

**Der Nachweis gilt nur zum Commit des Pakets.** In der Zusammenfassung steht immer nur der **letzte** Prüflauf einer Session — wer nach dem Commit noch einmal `checks.mjs run` startet, etwa als Handprobe, und ihn abbricht, hinterlässt eine Fassung, die den abgelieferten Stand gar nicht gemessen hat. Der Runner hält die Datei deshalb gegen den Commit der Session: Jede Datei, die der Commit ändert, muss darin mit genau dem Blob geprüft sein, der im Commit gelandet ist — dieselbe Frage, die das [Commit-Gate](#das-commit-gate) vor dem Commit gegen den Index stellt. Ein abgebrochener Lauf (`abgeschlossen: false`) zählt nie als Nachweis. Passt es nicht, **prüft der Runner selbst nach**: Er fährt die Paketstufe der `buildChecks` und meldet die Session als **`nachgeprueft`** (grün) oder **rot**, in beiden Fällen mit dem Grund in der Zeile („Nachweis gehörte nicht zum Commit …"). `nachgeprueft` ist ein Erfolg wie `geprueft` und wird am Leitstand ebenso grün eingefärbt. Hat die Session keinen Commit hinterlassen, gibt es nichts zu vergleichen — dann bleibt alles, wie es die Zusammenfassung sagt.
...
**Permissions.** Unbeaufsichtigt heißt: niemand beantwortet Permission-Dialoge. Der Runner startet die Sessions deshalb mit `--permission-mode auto --permission-prompts none` (Issue #940); alles Weitere erlaubst du gezielt über eine Allowlist in `.claude/settings.json` des Projekts. Die beiden Flags gehören zusammen: Im auto mode entscheidet ein Klassifikator, und damit wirkt deine Allowlist auch nachts — anders als im früheren Modus, der Edits pauschal annahm und dabei eine eingebaute Sperre für sensible Dateien mitführte, die kein `allow`-Eintrag aufhob. Was der Klassifikator nicht entscheiden kann, würde zur Rückfrage; `none` lehnt sie automatisch ab, statt sie an einen SDK-Host zu schicken, den der Runner nicht hat. Ohne `none` hinge die Session bis zum Rundenzeitlimit. Vor der ersten Session prüft der Runner in jeder Betriebsart, ob `.claude/settings.json`, `.claude/settings.local.json` und `~/.claude/settings.json` — soweit vorhanden — gültiges JSON sind, und stoppt bei einem Fehler hart mit Pfad und Parser-Meldung (im Dry-Run nur berichtet): Eine ungültige Datei setzt alle ihre Einstellungen außer Kraft, und nachts sähe man davon nur eine Genehmigungsabfrage, die niemand beantwortet. Die Allowlist z. B.:
...
**Die Geste:** das Label `kit:night` am gegroomten `[Fachlich]`-Issue im Backlog — oder an einem fertigen `[Plan]`-Dokument, siehe [Ein fertiger Plan als Auftrag](#ein-fertiger-plan-als-auftrag). Der Runner **verbraucht es beim Start** der Kette — jedes Setzen autorisiert genau eine Kette; ein Abbruch führt zu einem Bericht mit Grund und einer neuen Geste, nicht zur stillen Wiederholung. Der Labelname kommt aus `night.kette.label` und muss am Board einmal angelegt sein (GitHub `gh label create`, kanban-kit `POST /api/boards/{boardId}/labels`).

**Die Pruefung als Voraussetzung.** Das Kettenlabel allein genügt nicht mehr: Die gekennzeichnete Karte muss zu Beginn des Laufs zusätzlich das Label `review:fertig` tragen — die fachliche Anforderung ebenso wie das Plandokument eines Plan-Auftrags. Den Weg dahin geht `/issue-review <Nummer>`, das die Karte prüft und `review:fertig` setzt. Fehlt es, wird sie übersprungen: Sie behält ihr Kettenlabel, die Geste ist nicht verbraucht, und er bekommt einmal den Kommentar mit dem Anker `## Kette nicht gestartet: Pruefung fehlt`, der den Grund und den Weg über `/issue-review` nennt. Das ist ein anderer Fall als der Kommentar `Kette nicht gestartet` bei gescheitertem Reviewer-Vorflug: Dort waren die Reviewer nicht erreichbar, hier fehlt die Prüfung der Anforderung selbst — beide Kommentare beginnen mit demselben Text, sind aber an ihrem Anker zu unterscheiden. Wie `kit:night` und `kit:durchziehen` muss `review:fertig` am Board einmal angelegt sein; trägt keine einzige Karte es, meldet der Runner das im Protokoll.

**Die Variante:** ein zweites, unabhängiges Label an derselben Karte, `kit:durchziehen` (`night.kette.varianteBLabel`), kennzeichnet sie für Variante B. Gelesen wird es dort, wo auch `kit:night` steht — bei einem Plan-Auftrag also am Plan, nicht an der fachlichen Anforderung dahinter. Anders als `kit:night` wird es **nicht verbraucht** — es bleibt an der Karte stehen, denn es kennzeichnet die Karte selbst und nicht nur den einen Lauf; eine spätere Kette zu ihr (etwa nach `angehalten`) liest es erneut. Fehlt es, gilt Variante A. Wie `kit:night` muss es am Board einmal angelegt sein.
...
| umsetzung (nur Variante B) | `/implement-next #P` je Arbeitspaket, wortgleich mit der Umsetzungsnacht | Paket nach Ready gezogen, umgesetzt (In review) oder zurückgestellt; die Session erfährt von der Kette nichts, sie sieht ein reguläres Ready-Paket | Lock `night-umsetzung.lock` bereits gehalten **oder** Hauptkopie vor dem ersten Paket unsauber: Stufe ausgelassen, Rückfall auf Variante A, `unvollstaendig`; harter Stopp: `abgebrochen`; mindestens ein Paket hält an einer Stopp-Frage: `angehalten` (das Paket trägt `kit:klaeren` bereits selbst, der Fachplan bekommt keinen zweiten Halt) |
...
**Eine Kette ohne ausgeführte Umsetzung heißt nicht gelungen.** Lässt die Stufe `umsetzung` ihre Arbeit aus — weil eine andere Umsetzung den Lock hält oder die Hauptkopie vor dem ersten Paket unsauber ist —, endet die Kette auf dem Ausgang `unvollstaendig`, nicht auf `fertig`. Er steht zwischen den beiden: Die Kette lief durch und hat etwas Bestelltes nicht getan; sie hält niemanden auf (anders als `angehalten` wartet keine Frage auf dich) und es ist nichts gerissen (anders als `abgebrochen`). Am Board erscheint der Vorgang **gelb ohne Fehlerklasse**, den Grund trägt sein Auszug. Der Nachtbericht nennt unter `### Umsetzung` als erste Zeile `- ausgelassen: <grund> — die Pakete bleiben in Backlog.`, und der Grund steht mit dem Präfix `Umsetzung ausgelassen:` schon oben unter `### Ausgang`. Damit ist am Morgen ohne Blick ins Protokoll zu sehen, dass die Bestellung „zieh die Pakete gleich durch" nicht ausgeführt wurde, was sie verhindert hat und wo die Pakete jetzt liegen. Vorher war dieser Fall von einer gelungenen Nacht nicht zu unterscheiden. **Die Sperre selbst bleibt:** Zwei Umsetzungen in einem Arbeitsverzeichnis gehen nicht, die Kette wartet nicht auf das Freiwerden und fällt wie bisher sofort auf Variante A zurück. Geändert hat sich allein, was du danach siehst. Die Pakete ziehst du morgens selbst nach Ready.
...
`varianteBLabel` kennzeichnet die gekennzeichnete Karte für die Umsetzungsstufe (Variante B); `umsetzungMin` ist ihr Zeitbudget, `kostenUsdB` ihr eigener Kostendeckel.
...
**Der Rückweg nach `angehalten`.** Die Kette schreibt die eine Frage als Kommentar `## Kette angehalten` an die **gekennzeichnete Karte** — die, an der du abends das Label gesetzt hast — und setzt dort `kit:klaeren`; Plan und bis dahin geschnittene Pakete bleiben als Entwurf stehen. Der Weg nach vorn führt über den **Plan**, und der Kommentar zählt seine Schritte auf: die Antwort als Eintrag unter `## Architektonische Entscheidungen` des Plans, `## Offene Fragen` wieder auf `- Keine.` (ein Zusatz dahinter ist erlaubt), `/issue-review #M`, `kit:klaeren` am Plan und an der fachlichen Anforderung abnehmen, zuletzt `kit:night` an den Plan. Die nächste Kette nimmt ihn dann als **Plan-Auftrag** und fährt von der Stufe `pakete` an weiter. Die Antwort gehört **nicht** unter `## Offene Fragen`: Dort liest die Kette sie als weitere offene Frage und überspringt den Plan mit deinem eigenen Antworttext als Ausschlussgrund. Entsteht stattdessen ein neuer Plan (weil du erneut vom Fachplan aus startest), ist er der gültige; ältere Pläne zum selben Fachplan bekommen den Kommentar `Ueberholt durch Plan #M2` (kein Label, kein Move) und stehen im Bericht unter „Ueberholt". Jeden dieser Kommentare liest die Kette einmal zurück; findet sie ihn an der Karte nicht wieder, läuft sie weiter, führt den Plan aber nicht als überholt, sondern mit Grund unter „Ueberholt, nicht bestaetigt" — ein Bericht, der eine Handlung behauptet, die am Board niemand sieht, ist schlimmer als keiner. `kit:klaeren` nimmt ausschließlich der Mensch ab — ein Lauf, der sein eigenes `kit:klaeren` abräumen dürfte, könnte sich selbst freigeben.

**Kette und Umsetzung nebeneinander.** Die beiden Betriebsarten meinen verschiedene Karten und Spalten: `kit:night` am Fachplan im Backlog, `kit:nightrun` am Arbeitspaket in Ready. Ein Lauf ist immer genau eine Betriebsart (`--kette` oder nicht), aber zwei Läufe dürfen zur selben Zeit fahren — die Kette im Worktree, die Umsetzung in der Hauptkopie. Eine Einschränkung gilt unter **Variante B**: Ihre Stufe `umsetzung` baut wie die Umsetzungsnacht in der Hauptkopie, nicht im Worktree, und beide nehmen dafür denselben Lock `.claude/night-umsetzung.lock` — wer ihn zuerst hält, baut; der andere Lauf lässt seine Umsetzung aus (die Kette fällt dabei auf Variante A zurück, die übrigen Stufen bleiben unberührt). Unter Variante A gilt die Einschränkung nicht: Die Kette baut nie, und der Lock bleibt frei. **Der Zustand der Hauptkopie hält die Kette nicht auf:** Unkommittierte Reste — etwa aus einer parallel laufenden Umsetzungsnacht — prüft sie weder vorab noch nach der Vorflug-Session, weil sie die Hauptkopie gar nicht anfasst. Nur unter Variante B misst sie ihn einmal vor dem ersten Paket: Ist die Hauptkopie dann nicht sauber, lässt sie die Umsetzung **regulär aus** wie beim gehaltenen Lock — die Pakete bleiben in Backlog, der Grund mit den Dateinamen steht im Bericht, und der Lauf endet ohne Fehler. Bereinigen und die Pakete selbst nach Ready ziehen erledigt dann der Mensch.

#### Ein fertiger Plan als Auftrag

**Die Geste am Plan.** Die Kette nimmt nicht nur eine fachliche Anforderung an, sondern auch ein fertiges Plandokument: `kit:night` an einem `[Plan]`-Issue im Backlog startet einen **Plan-Auftrag**. Das ist der Weg zurück aus einem Halt (siehe unten) und zugleich der Weg, einen tagsüber von Hand geschriebenen Plan über Nacht zerlegen zu lassen.

**Die vier Bedingungen** sind dieselben wie bei der fachlichen Anforderung, nur an der anderen Karte gemessen: Der Plan steht im **Backlog**, trägt `review:fertig`, trägt **kein** `kit:klaeren` und hat eine erkennbare fachliche Herkunft — die Zeile `Fachliche Quelle: Issue #N` in seinem Kontext. Fehlt eine davon, wird der Plan mit Grund übersprungen und behält sein Kettenlabel; die Geste ist nicht verbraucht. Dazu zählt auch eine offene Frage im Dokument selbst: Unter `## Offene Fragen` muss `- Keine.` stehen (ein Zusatz dahinter ist erlaubt).

**Was entfällt.** Ein Plan-Auftrag fährt die Stufen `plan` und `review` **nicht** — der Plan ist geschrieben und geprüft. Der Lauf beginnt bei der Stufe `pakete` (`/issues #M`) und geht weiter wie sonst: Abdeckung gegen die fachliche Anforderung `#N`, unter Variante B die Umsetzung. Der Plan bleibt dabei unangetastet: Es entsteht **kein** zweites `[Plan]`-Dokument, seine Nummer wandert unverändert in die Herkunftszeile der Pakete und in den Bericht, und **kein** älterer Plan bekommt einen `Ueberholt durch Plan`-Kommentar. Im Bericht steht die Plan-Stufe deshalb als `als Auftrag uebernommen, nicht neu geschrieben` statt mit Dauer und Kosten.

**Die Variante am Plan.** `kit:durchziehen` liest die Kette an derselben Karte wie `kit:night` — bei einem Plan-Auftrag also am Plan. Ein `kit:durchziehen` an der fachlichen Anforderung dahinter bleibt dabei wirkungslos: Es gilt **Variante B** nur, wenn der Plan selbst gekennzeichnet ist.

**Zwei Kollisionsregeln** lösen auf, wenn mehr gekennzeichnet ist als nötig:

- **Die Anforderung weicht dem Plan.** Trägt eine fachliche Anforderung `#N` das Kettenlabel und zugleich ein aus ihr entstandener Plan, läuft der **Plan**; die Anforderung geht mit Grund nach `uebersprungen` und behält ihr Label. Das gilt auch, wenn der Plan selbst übersprungen wird — sonst plante die Nacht die Anforderung neu, während der fertige Plan danebenliegt.
- **Von zwei Plänen derselben Wurzel läuft der jüngere.** Tragen mehrere gekennzeichnete Pläne dieselbe fachliche Quelle, läuft der mit der höheren Nummer; der ältere geht mit Grund nach `uebersprungen`. Beide Gründe nennen die Nummer der Karte, die stattdessen läuft.

Ein Fachplan-Auftrag zu einer Anforderung und ein Plan-Auftrag zu einer anderen laufen dagegen in derselben Nacht beide — die Regeln greifen nur innerhalb derselben Wurzel. Eine weichende Karte verbraucht keinen `--max`-Platz.
...
Das Verlaufsprotokoll zeigt dir ohne Zutun jeden Tool-Aufruf der Session. Daran siehst du binnen Minuten, ob das Modell die Board-Operationen sauber hinbekommt — das ist der schnellste Machbarkeitstest, und er entscheidet die Frage, bevor du eine ganze Nacht investierst.
