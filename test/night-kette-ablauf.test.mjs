// Der glatte Durchlauf einer Nacht-Kette bis zum geprueften Plan (Plan #638; Issue #643).
//
...
import {
  NUR_POSIX, run, board, mitProjekt, fachplan, umgebung, sessions, stand, PLAN_ANLEGEN, REVIEW_MARKER,
...
test("[night-19] eine Kette laeuft bis zum geprueften Plan: Label weg, fertig, Worktree entfernt, Notiz zurueck", NUR_POSIX, () => {
  mitProjekt((dir) => {
    ...
    const env = umgebung(dir, { stufen: { plan: PLAN_MIT_NOTIZ, review: REVIEW_MARKER } });
    ...
    assert.equal(lauf.kostenSumme, 2);
    ...
    assert.deepEqual(gelaufen.map((s) => s.stufe), ["plan", "review"]);
