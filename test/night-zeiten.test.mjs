...
//
// Zwei Ebenen werden geprueft: `zeitenBauen()` ist die reine Feldkonstruktion (wie
// `verbrauchAddieren`), direkt an Fixtures pruefbar. Das Schreiben auf die Einheit
...
// Aufruf fuer dieselbe Karte (Salvage) trifft dieselbe, juengste Einheit und
// ueberschreibt ihre Zeiten.
...
import { zeitenBauen } from "../kit/night.mjs";
...
test("[night-51] eine 0 bleibt eine 0, kein `|| null`", () => {
  ...
  assert.deepEqual(zeitenBauen(0, kennzahlen, werkzeug), {
    dauerMs: 0, nachdenkenMs: 0, werkzeugMs: 0, werkzeugSchuebe: 0, nebenlaeufigeSchuebe: 0,
  });
});
...
test("[night-51] nach einer Session mit Kennzahlen und Beobachter-Ergebnis traegt die Einheit zeiten mit allen fuenf Feldern", NUR_POSIX, () => {
  ...
  try {
    ...
    assert.deepEqual(Object.keys(z).sort(), ["dauerMs", "nachdenkenMs", "nebenlaeufigeSchuebe", "werkzeugMs", "werkzeugSchuebe"]);
...
test("[night-51] laeuft dieselbe Karte mehrfach (Salvage), trifft es die juengste Einheit und ueberschreibt ihre Zeiten", NUR_POSIX, () => {
  ...
  try {
    ...
    const z = einheit(dir, id).zeiten;
    assert.equal(z.nachdenkenMs, 9000, "die juengste (Salvage-)Session ueberschreibt die Zeiten der ersten");
